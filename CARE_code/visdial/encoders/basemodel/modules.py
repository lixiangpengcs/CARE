...
class ModalityFusionTopic(nn.Module):
    def __init__(self, hparams):
        ...
        self.MLP = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(hparams.img_feature_size + hparams.word_embedding_size * 2, hparams.img_feature_size),
            # 2648 -> 2048
            nn.ReLU(),
        )
    ...
    def forward(self, img, topic_aggregation, ques_not_pad):
        ...
        vt_att = torch.sum(
            vt_score.unsqueeze(-1) * topic_aggregation.reshape(bs * num_r, sl_q, topic_agg_size).unsqueeze(1), dim=2)
...
class TextAttImage(nn.Module):
    ...
    def forward(self, image, context_matching, img_mask=None):
        image = image.unsqueeze(1).repeat(1, 10, 1, 1)

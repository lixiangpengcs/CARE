...
class QuesModalityFusionTopic(nn.Module):
	def __init__(self, hparams):
		super(QuesModalityFusionTopic, self).__init__()
		self.hparams = hparams
		...
		self.MLP = nn.Sequential(
			nn.Dropout(p=hparams.dropout_fc),
			nn.Linear(hparams.hidden_size + hparams.word_embedding_size * 2,
								hparams.hidden_size + hparams.word_embedding_size * 2),
			nn.ReLU(),
			nn.Linear(hparams.hidden_size + hparams.word_embedding_size * 2, hparams.hidden_size), # 2648 -> 2048
			nn.ReLU(),
		)
	...
	def forward(self, img, topic_aggregation, ques_not_pad):
		...
		bs, _, num_p, img_feat_size = img.size()
		_, num_r, sl_q, topic_agg_size = topic_aggregation.size()
		lstm_size = self.hparams.lstm_hidden_size

		v_feat = self.img_nonlinear_emb(img) 	# bs np lstm
		v_feat = v_feat.reshape(bs*num_r,num_p, lstm_size)
		...
		# bs*num_r np img_feat
		img_expanded = img.reshape(bs*num_r, num_p, img_feat_size)
		mf_topic = self.MLP(torch.cat((img_expanded, vt_att), dim=-1)) # 2048 + 600
		mf_topic = mf_topic.reshape(bs, num_r, num_p, img_feat_size)
